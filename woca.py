# German C1 TELC Flashcard App (v14 - UX/UI 개선 버전)
# 독일어 C1 TELC 준비용 플래시카드 앱 (v14 - 사용자 경험 개선 버전)
...
<style>
    /* 카드 기본 스타일 */
    .flashcard-front, .flashcard-back-container {
        background: white;
        color: #333;
    ...
    div[data-testid="stButton"] > button {
        ...
        width: 100%;
        color: inherit;
        text-align: inherit; /* 내부 콘텐츠의 정렬을 따르도록 설정 */
    }
    div[data-testid="stButton"] > button:hover,
    div[data-testid="stButton"] > button:active,
    div[data-testid="stButton"] > button:focus,
    div[data-testid="stButton"] > button:focus:not(:active) {
        background-color: #f8f9fa; /* 마우스를 올렸을 때 미세한 피드백 */
        color: inherit;
        border: none;
        box-shadow: none;
        border-radius: 15px; /* 카드 모양과 일치 */
    }
</style>
...
def standardize_columns(df):
    ...
    column_candidates = {
        ...
        'ko_example_translation': ['ko_example_translation', 'korean_example_translation', 'example_ko', '예문번역', '예문해석'], # ✨ 요청사항 반영
...
def generate_card_html(row, mapping, is_answer_side):
    """요청에 따라 카드 앞면 또는 뒷면의 전체 HTML을 생성합니다."""
    # --- Card Front (Question) ---
    if not is_answer_side:
        ...
        return f"""<div class="flashcard-front">
    ...
    # --- Card Back (Answer) ---
        ...
        html_parts.append(f"""<div class="flashcard-back-container">
                                  <div class="german-word">{german_word}</div>
        ...
        if "verb" in pos.lower() or "Verb" in pos:
            ...
            if complement_structure:
                html_parts.append(f'<div class="case-structure"><strong>📝 문장 구조:</strong> <code>{complement_structure}</code></div>')
                ...
                if explanations: html_parts.append(f'<div class="grammar-explanation">{"<br/>".join(explanations)}</div>')
            
            if prep:
                html_parts.append(f'<div class="grammar-explanation"><strong>🔗 전치사:</strong> <code>{prep}</code><br/>{get_prep_explanation(prep)}</div>')
            elif case and not complement_structure and get_case_explanation(case):
                html_parts.append(f'<div class="grammar-explanation"><strong>📋 격 지배:</strong> {case}<br/>{get_case_explanation(case)}</div>')
        
        elif "Nomen-Verb" in pos:
            complement_structure = safe_get(row, 'complement_structure', mapping)
            if complement_structure:
                html_parts.append(f'<div class="case-structure"><strong>📝 명사-동사 구조:</strong> <code>{complement_structure}</code></div>')
        ...
        if grammar_info_list:
            ...
            html_parts.append(f'<div class="grammar-info"><div class="grammar-title">📚 추가 정보</div><ul>{info_html}</ul></div>')
